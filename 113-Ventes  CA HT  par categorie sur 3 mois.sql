...
select
    ...
    sum(case when "per_month_total"."month" = -3 then "per_month_total"."ca" else 0 end) as "CA HT m-3",
    sum(case when "per_month_total"."month" = -2 then "per_month_total"."ca" else 0 end) as "CA HT m-2",
    sum(case when "per_month_total"."month" = -1 then "per_month_total"."ca" else 0 end) as "CA HT m-1",
    sum(case when "per_month_total"."month" = 0 then "per_month_total"."ca" else 0 end) as "CA HT m"
from
    (
    select
        ...
        sum("poo".price_total_vat_excl) as "ca"
    from
